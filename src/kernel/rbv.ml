...
let rec rbV v = match v with
  ...
  | VIndW e              -> EIndW (rbV e)
  | VSum s               -> List.fold_left eApp (EVar (ident s.name)) (List.map rbV s.params)
  | VCon c               -> List.fold_left eApp (EVar (ident c.cname)) (List.map rbV (List.append c.parent.params c.cparams))
