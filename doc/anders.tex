...
operation which depends on hcomp primitive. Here is given the first part of Huber equations for \textbf{transp}.
...
encoding of CW-complexes. Here is given the second part of Huber equations for $\textbf{hcomp}$.
...
\textbf{def} iso-Intro (A B: U) : iso-Form A B := \ (x : iso A B), isoPath A B x.f x.g x.s x.t \\
